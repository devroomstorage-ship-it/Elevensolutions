  ...
  return (
    <aside className="w-48 bg-[#3A2150] flex flex-col border-r border-white/10 flex-shrink-0">
      ...
      <nav className="flex-1 py-3 overflow-y-auto">
        <p className="px-4 text-[10px] text-[#D7A8CE] font-semibold tracking-widest mb-2 uppercase">Main</p>
        ...
        <p className="px-4 text-[10px] text-[#D7A8CE] font-semibold tracking-widest mb-2 mt-5 uppercase">System</p>
          ...
          return (
            <Link key={item.href} href={item.href}
              className={`flex items-center gap-2.5 px-4 py-2 text-xs transition-colors border-l-2 ${
                active ? 'text-white border-[#B060A0]' : 'text-[#D7A8CE] hover:text-white border-transparent'
              }`}>
              {item.label}
            </Link>
          );
      ...
      <div className="p-3 border-t border-white/10">
        <div className="flex items-center gap-2 mb-2">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[#D7A8CE] text-[10px] capitalize">{user?.role?.replace('_',' ')}</p>
          </div>
        </div>
